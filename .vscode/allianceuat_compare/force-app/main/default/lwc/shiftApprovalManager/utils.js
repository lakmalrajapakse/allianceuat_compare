...
export default class Utils {
    ...
    static convertShifts(records = [], siteTimezones, shiftSignatures = {}) {
        let results = [];
        for (let record of records) {
            ...
            let esignImages = Utils.getImages(record?.ContentDocumentLinks?.records ?? []); 
            let shift = {
                Id: record.Id,
                shift: record,
                ...
                approvedStart: hasApprovedStart ? record?.sirenum__Billable_Start_Time__c : (hasActualStart ? record?.sirenum__Actual_Start_Time__c : record?.sirenum__Scheduled_Start_Time__c),
                approvedEnd: hasApprovedEnd ? record?.sirenum__Billable_End_Time__c : (hasActualEnd ? record?.sirenum__Actual_End_Time__c : record?.sirenum__Scheduled_End_Time__c),
                wfmId: record?.WFM_External_Reference_ID__c ?? '',
                timezone: siteTimezones[record?.sirenum__Site__c] ?? 'Australia/Sydney',
                ...
                //hasImages: !!record?.ContentDocumentLinks?.records?.length,
               // images: Utils.getImages(record?.ContentDocumentLinks?.records ?? []),
               
                hasImages: esignImages.length > 0,
                images: esignImages,
            ...
            if (shift.hasBreaks) {
                for (let sbreak of shift.breaks) {
                    let { hoursDecimal, hours, minutes, formattedString } =
                        this.calculateLength(sbreak.sirenum__Start_Time__c, sbreak.sirenum__End_Time__c, sbreak);
                    ...
                    console.log('SPC totalBreakHours:', totalBreakHours);
                    console.log('SPC totalBreakMins:', totalBreakMins);
                }
  ...
  /*  static getImages(fileAttachments) {
        let images = [];
        for (let attachment of fileAttachments) {
            if (attachment.ContentDocument.FileExtension == 'jpg' || attachment.ContentDocument.FileExtension == 'jpeg' || attachment.ContentDocument.FileExtension == 'png') {
                let imageName = attachment.ContentDocument.Title + '.' + attachment.ContentDocument.FileExtension;
                console.log('SPC utils getImages imageName', imageName);
                images.push({ link: '/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB240BY180&versionId=' + attachment.ContentDocument.LatestPublishedVersionId, name: imageName });
            }
        }
        return images;
    }*/
   static getImages(fileAttachments) {
    ...
    for (let attachment of fileAttachments) {
        ...
        let title = attachment.ContentDocument.Title || '';
        let ext = attachment.ContentDocument.FileExtension;

        // Only allow ESign files
        if (
            title.startsWith('ESign') &&
    ...
    static convertExpenses(expenses = []) {
        ...
        for (let expense of expenses) {
            let newExpense = {
                ...
                'Amount': expense?.sirenum__Amount__c ?? '£0.00',
